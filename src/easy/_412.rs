...
impl Solution {
    ...
    pub fn fizz_buzz_unsafe_optimized(n: i32) -> Vec<String> {
        ...
        let n_us = n as usize;
        // Pre-allocate result Vec for exactly n_us elements
        let mut result: Vec<String> = Vec::with_capacity(n_us);
        // SAFETY: we'll write directly into this buffer
        let ptr = result.as_mut_ptr();

        // Use counters instead of modulo for speed
        let mut fizz_count = 3;
        let mut buzz_count = 5;

        // For each value from 1 to n_us
        for i in 1..=n_us {
            // Decrement counters
            fizz_count -= 1;
            buzz_count -= 1;
            ...
            if fizz_count == 0 && buzz_count == 0 {
                ...
                let lit = "FizzBuzz";
                let mut tmp = String::with_capacity(lit.len());
                tmp.push_str(lit);
                s = tmp;
                ...
                let slice = unsafe { std::str::from_utf8_unchecked(&buf[..len]) };
        ...
        // SAFETY: all n_us elements initialized, set Vec length
        unsafe {
            result.set_len(n_us);
        }
