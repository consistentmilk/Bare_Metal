...
impl Solution {
    pub fn maximum_triplet_value(nums: Vec<i32>) -> i64 {
        ...
        for &num in nums.iter() {
            let icurr: i64 = num as i64;

            max_val = std::cmp::max(max_val, dmax * icurr);
            dmax = std::cmp::max(dmax, imax - icurr);
            imax = std::cmp::max(imax, icurr);
        }
...
mod tests {
    ...
    fn run_all_implementations(nums: Vec<i32>) -> Vec<i64> {
        vec![Solution::maximum_triplet_value(nums)]
    }
